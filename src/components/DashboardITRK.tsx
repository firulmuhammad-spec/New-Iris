...
export default function DashboardITRK({ currentUser, onLogout, allData, onDataRefresh }: DashboardITRKProps) {
  ...
  const handleUpdateStandard = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch(`/api/master/standards/${editingStandard.id}`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
  ...
  const handleUpdateSignature = async (e?: React.FormEvent) => {
    ...
    try {
      const res = await fetch(`/api/master/signatures/${editingSignature.id}`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          name: editSigName,
  ...
  const handleConfirmBatchRegister = async () => {
    ...
    try {
      const res = await fetch("/api/registrations/batch", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ items: batchPreviewItems })
      });
      const data = await res.json();
      if (data.success) {
        setBatchPreviewItems([]);
        alert(`Sukses mendaftarkan ${data.count} sampel baru secara batch ke antrean draf!`);
        refreshData();
      } else {
        alert(data.message || "Gagal memproses batch.");
      }
    } catch (err) {
      console.error(err);
  ...
  const handleBulkDeleteDrafts = async () => {
    ...
    try {
      ...
      console.error(err);
      alert("Gagal menghapus beberapa draft.");
    } finally {
      setAiScanning(false);
    }
  };
  ...
  const handleBulkSubmitDraftsToUji = async () => {
    ...
    try {
      ...
      console.error(err);
      alert("Gagal memproses bulk submit.");
    } finally {
      setAiScanning(false);
    }
  };
  ...
  const handleBulkReviewApprove = async (approve: boolean) => {
    ...
    try {
      ...
      console.error(err);
      alert("Gagal memproses bulk review.");
    } finally {
      setAiScanning(false);
    }
  };
  ...
  const handleManualRegisterSub = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data.success) {
        ...
        alert(data.message || "Gagal menyimpan sampel.");
      }
    } catch (err) {
      console.error(err);
  ...
  const saveKarungGridResults = async () => {
    ...
    try {
      const res = await fetch("/api/registrations/update-results", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          id: activeTesting.id,
      ...
      if (data.success) {
        ...
        // Do not redirect to review screen after testing per user request
        // setActiveTab("review");
        refreshData();
        ...
        alert("Gagal menyimpan hasil uji.");
      }
    } catch (err) {
      console.error(err);
      alert("Terjadi kesalahan sistem saat menyimpan hasil.");
    }
  };
  ...
  const handleReviewDecision = async (regId: string, approved: boolean, overrideQrSignature?: boolean) => {
    ...
    try {
      const res = await fetch("/api/registrations/review", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          id: regId,
                  ...
                  <button
                    onClick={async () => {
                      ...
                      try {
                        ...
                        const data = await res.json();
                        if (data.success) {
                          alert("Sandi berhasil diperbarui dan disinkronkan ke server lokal!");
                          input.value = "";
                        }
                      } catch {
